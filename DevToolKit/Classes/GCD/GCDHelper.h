...
/** 主线程运行 */
FOUNDATION_EXTERN void Execute_Main(dispatch_block_t main);

/** 异步加载 */
FOUNDATION_EXTERN void Execute_Async(dispatch_block_t async);

/** 延迟加载 */
FOUNDATION_EXTERN void Execute_Delay(CGFloat seconds ,dispatch_block_t delay);

/** 使用GCD加线程锁  操作完成后加上 dispatch_semaphore_signal(sema) */
FOUNDATION_EXTERN void Execute_Lock_GCD(void (^lock)(dispatch_semaphore_t sema));

/** 使用OSSpinLock加线程锁 */
FOUNDATION_EXTERN void Execute_Lock_OSSpin(dispatch_block_t lock);

/** block内代码块执行所花费的时间 */
FOUNDATION_EXTERN void Execute_TakeTime (dispatch_block_t block, NSString *message);
